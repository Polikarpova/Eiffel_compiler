...
Expression* fromRefnCall(Method* mtd, struct NExpr* node)
{
	...
	if(node->left == NULL)	// единственный вариант дл€ локальной переменой: без квалификации
	{
		...
		if(loc_var)
		{
			...
			else	// единственный вариант дл€ локальной переменой: без аргументов
			{
...
MethodCall* fromPrecursor(Method* mtd, struct NExpr* node)
{
	...
	if(node->left != NULL) {	//  квалификаци€ не нужна дл€ PRECURSOR
		EiffelProgram::currentProgram->logError(
			QString("semantic"), 
...
/*virtual*/ void Expression::setConversionTo(EiffelType* newType)
{
	...
	*/
	...
	{
		////this->type = NULL; // сбросить, чтобы пересчитать тип выражени€ заново (не надо)
		this->castI2F = false; // сбросить (преобразование: INT к FLOAT)

		if(newType->isReal())
		{
			// установить новый тип выражени€
			////this->type = newType; // ??
...
EiffelType* Expression::getReturnType() {
	...
	//так же если ранее это функци€ вызывалась дл€ текущего объекта, то он уже знает о своЄм типе и нет надобности оп€ть его высчитывать
	if( type == NULL ) {
